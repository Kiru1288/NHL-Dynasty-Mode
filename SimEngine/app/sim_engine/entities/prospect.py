...
LEAGUE_PROFILES: Dict[LeagueType, LeagueProfile] = {
    ...
    LeagueType.PREP: LeagueProfile(LeagueType.PREP, 0.42, 0.32, 0.58, 0.55, 0.52),
...
class Prospect:
    ...
    # post-draft assignment (set when drafted)
    team_id: Optional[str] = None
    status: str = "undrafted"  # "undrafted" | "prospect" (in pipeline)
    development_league: str = "junior"  # CHL, NCAA, AHL, Europe, development
    development_years_remaining: int = 2  # years until promotion eligible
    ...
    def convert_to_player_payload(
        ...
        *,
        ...
        # When engine set draft_value_range (tiers), use it for projected so rookie OVR reflects tiers
        if getattr(self, "draft_value_range", None) and len(self.draft_value_range) >= 2:
            lo, hi = self.draft_value_range[0], self.draft_value_range[1]
            if isinstance(lo, (int, float)) and isinstance(hi, (int, float)) and hi > lo:
                projected = clamp(rng.uniform(float(lo), float(hi)))
