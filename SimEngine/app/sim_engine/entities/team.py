...
class PerformanceMemory:
    ...
    consecutive_bottom5_finishes: int = 0

    def record_season(self, win_pct: float, made_playoffs: bool, series_wins: int, won_cup: bool, standings_rank: Optional[int] = None):
        ...
        if standings_rank is not None and standings_rank > 0:
            if standings_rank >= 28:
                self.consecutive_bottom5_finishes += 1
            else:
                self.consecutive_bottom5_finishes = 0
        else:
            self.consecutive_bottom5_finishes = 0
...
class Team:
    ...
    def __init__(
        ...
        *,
        ...
        self.prospect_pipeline_score: float = 0.0
        self.momentum_score: float = 0.5
    ...
    def seasonal_strength(self, coach_mod: float = 1.0, cap_eff: float = 1.0) -> float:
        base_strength = self.roster_quality.strength_score()
        pipeline = getattr(self, "prospect_pipeline_score", 0.0) * 0.25
        momentum = getattr(self, "momentum_score", 0.5)
        coach_term = (coach_mod - 1.0) * 0.15
        cap_term = (cap_eff - 1.0) * 0.10
        return max(0.0, min(1.0, base_strength + pipeline + (momentum - 0.5) * 0.20 + coach_term + cap_term))
    ...
    def _infer_status(
        self,
        win_pct: float,
        league_health: Optional[float] = None,
        cap_dip: bool = False,
        cap_spike: bool = False,
    ) -> str:
        """
        ...
        consecutive_bottom5 = getattr(self.memory, "consecutive_bottom5_finishes", 0)
        if consecutive_bottom5 >= 3:
        ...
        # Rebuild/retool: causal (cap_dip, league_health) + patience
        if roll < 0.48:
            if rebuild_bias > 0.10 or self.ownership.patience > 0.55 or self.archetype in (TeamArchetype.DRAFT_AND_DEVELOP, TeamArchetype.PATIENT_BUILDER):
                return TeamStatus.REBUILDING
    ...
    def update_team_state(self, *, win_pct: float):
        ...
        self.memory.record_season(
            ...
            series_wins=0,
            won_cup=False,
            standings_rank=getattr(self, "_last_standings_rank", None),
        )

        self._update_roster_quality_proxy(win_pct)

        prev_status = self.state.status
        self.state.status = self._infer_status(
            win_pct,
            league_health=getattr(self, "_last_league_health", None),
            cap_dip=getattr(self, "_last_cap_dip", False),
            cap_spike=getattr(self, "_last_cap_spike", False),
        )
        if self.state.status != prev_status:
            self.franchise_memory.append(f"STATUS_CHANGE:{prev_status}->{self.state.status}")

        rank = getattr(self, "_last_standings_rank", None)
        if rank is not None and rank >= 1 and rank <= 5:
            self.momentum_score += 0.05
        if rank is not None and rank >= 28:
            self.momentum_score -= 0.03
        self.momentum_score += (win_pct - 0.5) * 0.08
        if getattr(self, "_coach_turnover_this_year", False):
            self.momentum_score -= 0.06
        if getattr(self, "_era_mismatch", 0.0) > 0.5:
            self.momentum_score -= 0.04
        self.momentum_score = max(0.0, min(1.0, self.momentum_score))
...
def _avg_roster_age(team: Any) -> float:
    roster = list(getattr(team, "roster", None) or [])
    if not roster:
        return 27.0
    ages: List[int] = []
    for p in roster:
        idn = getattr(p, "identity", None)
        if idn is not None and hasattr(idn, "age"):
            try:
                ages.append(int(getattr(idn, "age")))
                continue
            except (TypeError, ValueError):
                pass
        try:
            ages.append(int(getattr(p, "age", 27)))
        except (TypeError, ValueError):
            ages.append(27)
    return float(sum(ages)) / float(max(1, len(ages)))


def update_team_gm_strategic_profile(
    team: Any,
    *,
    runner_archetype: str = "",
    point_pct: float = 0.5,
    standings_bucket: str = "bubble",
    pipeline_score: float = 0.5,
    cap_pressure: str = "moderate",
    rng: Optional[random.Random] = None,
    avg_roster_age: Optional[float] = None,
) -> None:
    """
    Derive competitive window + strategy from identity, results, age curve, and pipeline.
    Sets: gm_window, window, gm_strategy, gm_risk_band.
    Note: ``team.strategy`` is reserved for cap-state labels (see engine.update_team_strategy); GM posture lives in gm_strategy.
    """
    arch = str(runner_archetype or "").lower()
    bucket = str(standings_bucket or "bubble").lower()
    pt = float(point_pct)
    pipe = float(pipeline_score)
    av_age = float(avg_roster_age) if avg_roster_age is not None else _avg_roster_age(team)
    prev = float(getattr(team, "_gm_prev_point_pct", pt) or pt)
    trend_down = pt < prev - 0.034
    setattr(team, "_gm_prev_point_pct", pt)

    window = "emerging"
    if "rebuild" in arch or "tank" in arch or bucket == "rebuild" or pt < 0.43:
        window = "rebuild"
    elif pt >= 0.58 or bucket == "contender":
        window = "contender"
    elif (pt < 0.48 and trend_down and av_age > 27.0) or (av_age > 29.5 and pt < 0.52 and pipe < 0.52):
        window = "declining"
    if "win_now" in arch and window == "emerging" and pt > 0.535:
        window = "contender"
    if "draft_and_develop" in arch and window == "contender" and pt < 0.50:
        window = "emerging"
    if "rebuild" in arch or "tank" in arch:
        window = "rebuild"

    if window == "rebuild":
        strategy = "draft_focus"
    elif window == "contender":
        strategy = "aggressive" if ("chaos" in arch or "win_now" in arch) else "win_now"
    ...
    try:
        rf = float(getattr(team, "risk_tolerance", 0.55) or 0.55)
    except (TypeError, ValueError):
        rf = 0.55
    ...
    if rf >= 0.62:
        ...
        gm_risk = "medium"
    if rng is not None and getattr(team, "archetype", None) == TeamArchetype.CHAOTIC and rng.random() < 0.18:
        gm_risk = "high"

    cp = str(cap_pressure or "moderate").lower()
    if cp in ("cap_hell", "critical") and window == "contender":
        strategy = "conservative"
    if cp in ("low",) and window == "rebuild" and rng is not None and rng.random() < 0.35:
        strategy = "balanced"

    setattr(team, "gm_window", window)
    setattr(team, "window", window)
    setattr(team, "gm_strategy", strategy)
    setattr(team, "gm_risk_band", gm_risk)
