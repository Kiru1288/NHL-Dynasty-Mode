...
class League:
    ...
    def __init__(
        ...
        # Global asset registry: prevents duplicate waiver claims
        self.asset_registry: Dict[str, AssetState] = {}
        self.waiver_pool: List[str] = []

        # Persistent global player ecosystem (non–NHL-owned). SimEngine owns lifecycle.
        self.global_player_pool: List[Any] = []

    @property
    def global_prospect_pool(self) -> List[Any]:
        """Backward-compatible alias for global_player_pool."""
        return self.global_player_pool

    @global_prospect_pool.setter
    def global_prospect_pool(self, v: Any) -> None:
        self.global_player_pool.clear()
        if isinstance(v, list):
            self.global_player_pool.extend(v)

    def register_asset(self, asset_id: str, current_team_id: str, asset_type: str = "player") -> None:
        if asset_id not in self.asset_registry:
            self.asset_registry[asset_id] = AssetState(
                current_team_id=current_team_id,
                on_waivers=False,
                last_movement_year=0,
                asset_type=asset_type,
                is_active=True,
            )

    def place_on_waivers(self, asset_id: str, current_team_id: str, year: int, asset_type: str = "player") -> None:
        self.register_asset(asset_id, current_team_id, asset_type)
        self.asset_registry[asset_id].on_waivers = True
        self.asset_registry[asset_id].last_movement_year = year
        if asset_id not in self.waiver_pool:
            self.waiver_pool.append(asset_id)

    def claim_asset(self, asset_id: str, claiming_team_id: str, year: int) -> bool:
        if asset_id not in self.asset_registry or not self.asset_registry[asset_id].on_waivers:
            return False
        self.asset_registry[asset_id].current_team_id = claiming_team_id
        self.asset_registry[asset_id].on_waivers = False
        self.asset_registry[asset_id].last_movement_year = year
        if asset_id in self.waiver_pool:
            self.waiver_pool.remove(asset_id)
        return True

    def clear_waiver_pool(self) -> None:
        for asset_id in list(self.waiver_pool):
            if asset_id in self.asset_registry:
                self.asset_registry[asset_id].on_waivers = False
        self.waiver_pool.clear()

    def is_asset_available_for_claim(self, asset_id: str, year: int) -> bool:
        if asset_id not in self.asset_registry:
            return True
        return self.asset_registry[asset_id].on_waivers
    ...
    def get_era_modifiers_for_simulation(self) -> Dict[str, float]:
        """
        Era impacts: player valuation weights, coach demand, AAV curves, waiver frequency, scoring variance.
        """
        era_def = self.get_active_era_definition()
        intensity = self.era_state.intensity
        base = 1.0
        return {
            "goalie_weight_delta": lerp(0.0, (era_def.value_goalie_elite - 0.50) * 0.40, intensity),
            "offense_weight_delta": lerp(0.0, (era_def.value_speed_skill - 0.50) * 0.30, intensity),
            "defensive_value_delta": lerp(0.0, (era_def.value_two_way - 0.50) * 0.20, intensity),
            "waiver_frequency_delta": lerp(0.0, (era_def.contract_inflation_mod - base) * 0.15, intensity),
            "scoring_variance_mod": era_def.scoring_rate_mod,
            "contract_inflation_mod": era_def.contract_inflation_mod,
        }
