...
def _life_pressure_dict(player: Any) -> Dict[str, float]:
    lp = getattr(player, "life_pressure", None)
    if lp is None:
        return {}
    if isinstance(lp, dict):
        return {str(k): float(v) for k, v in lp.items()}
    if hasattr(lp, "__dict__"):
        return {str(k): float(v) for k, v in lp.__dict__.items()}
    return {}
...
def _build_retirement_context(
    ...
    *,
    ...
    from app.sim_engine.franchise.engine import _contract_years_remaining
...
def _serialize_retirement_row(
    ...
    *,
    ...
    row: Dict[str, Any] = {
        ...
        "draft_info": dict(getattr(player, "draft_info", None) or getattr(player, "draft", None) or {}),
...
def _apply_team_retirement_effects(session: FranchiseSession, team: Any, player: Any, row: Dict[str, Any]) -> None:
    from app.sim_engine.franchise.engine import player_cap_hit_millions

    pos = _player_position(player) or "F"
    needs = dict(getattr(team, "needs", None) or {})
...
def _confirm_retirement(
    ...
    from app.sim_engine.franchise.engine import _strip_retired_from_nhl_rosters
...
def _run_depth_retirement_pass(session: FranchiseSession, rng: Any) -> List[Dict[str, Any]]:
    ...
    from app.sim_engine.franchise.engine import _purge_retired_from_extra_pools
