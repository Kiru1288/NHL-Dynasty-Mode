...
# =============================================================================
# League/object integration wrapper (used by engine.py / run_sim.py)
# =============================================================================

class WaiverAI:
    """
    Wrapper that operates on live league/team/player objects.

    Expected (flexible) shapes:
    - league.teams: list of Team objects with .team_id, .roster
    - league.waiver_wire: list of {"player": Player, "from_team": team_id}
    - league.waiver_priority: list[str] (optional; built from standings/teams if absent)
    - team.cap_space_m or team.cap_space (optional)
    """

    def __init__(self, early_season_cutoff_day: int = 30):
        self.engine = WaiverEngine(WaiverConfig(early_season_cutoff_day=early_season_cutoff_day))

    @staticmethod
    def _team_dict(team: Any) -> TeamDict:
        tid = str(getattr(team, "team_id", getattr(team, "id", "")))
        # weak teams first: treat missing points as 0 (bad)
        return {
            "team_id": tid,
            "points": getattr(team, "points", 0),
            "point_pct": getattr(team, "point_pct", 0.0),
            "goal_diff": getattr(team, "goal_diff", 0),
            "cap_space": getattr(team, "cap_space_m", getattr(team, "cap_space", 10.0)),
            "competitive_window": getattr(team, "status", getattr(team, "archetype", "bubble")),
            # if team.needs exists, pass positional needs as a list for waiver module bonus
            "roster_needs": WaiverAI._needs_to_roster_needs(getattr(team, "needs", None)),
            "roster_size": len(getattr(team, "roster", None) or []),
            "roster_limit": 23,
        }

    @staticmethod
    def _needs_to_roster_needs(needs: Any) -> List[str]:
        if not isinstance(needs, dict):
            return []
        out: List[str] = []
        # map needs buckets to positions for waiver module
        ...
        if float(needs.get("top_line_forward", 0.0)) >= 0.55:
            out.append("C")
            out.append("W")
        if float(needs.get("depth_forward", 0.0)) >= 0.65:
            out.append("W")
    ...
    def _player_dict(player: Any) -> PlayerDict:
        ...
        return {
            ...
            "position": getattr(getattr(player, "position", None), "value", getattr(player, "position", "")),
    ...
    def process_waivers(self, league: Any, *, season_day: int = 1) -> List[str]:
        ...
        if not teams:
            return logs
        wire = list(getattr(league, "waiver_wire", None) or [])
        if not wire:
            return logs

        # build team dicts for waiver engine
        team_dicts: List[TeamDict] = [self._team_dict(t) for t in teams]

        # build/keep priority
        priority = list(getattr(league, "waiver_priority", None) or [])
        if not priority:
            # worst-first using current points (if present)
            priority = build_waiver_priority_from_standings(team_dicts)

        # id->team
        by_id = {str(getattr(t, "team_id", getattr(t, "id", ""))): t for t in teams}

        # process each waived player in order
        remaining_wire = []
        for item in wire:
            p = item.get("player")
            from_team_id = str(item.get("from_team", ""))
            if p is None:
                continue
            winner, priority = self.engine.process_player_and_update_priority(
                player=self._player_dict(p),
                teams=team_dicts,
                priority_order=priority,
            )
            if winner is None:
                remaining_wire.append(item)
                continue

            to_team = by_id.get(str(winner))
            if to_team is None:
                remaining_wire.append(item)
                continue
            ...
            # mutate rosters
            try:
                if from_team_id and from_team_id in by_id:
                    fr = by_id[from_team_id]
                    fr_roster = list(getattr(fr, "roster", None) or [])
                    if p in fr_roster:
                        fr_roster.remove(p)
                        fr.roster = fr_roster
                to_roster = list(getattr(to_team, "roster", None) or [])
                to_roster.append(p)
                to_team.roster = to_roster
                logs.append(f"WAIVERS: {getattr(to_team, 'name', winner)} claims {getattr(p, 'name', 'Player')} from {from_team_id or 'waivers'}")
            except Exception:
                remaining_wire.append(item)

        # persist updated priority + wire
        try:
...
_DEFAULT_WAIVER_AI = WaiverAI()


def process_waivers(league: Any) -> List[str]:
    return _DEFAULT_WAIVER_AI.process_waivers(league)
