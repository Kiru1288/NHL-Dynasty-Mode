...
def normalize_contract_dict(raw: Any) -> Dict[str, Any]:
    ...
    ctype = str(src.get("contract_type") or src.get("type") or "STANDARD").upper()
    out["type"] = ctype
...
# ---------------------------------------------------------------------------
# Full cap snapshot — THE single cap reality
# ---------------------------------------------------------------------------
...
def compute_market_value(player: Any, league: Any = None) -> float:
    ...
    base = LEAGUE_MINIMUM_AAV_M + max(0.0, ovr - 58.0) * 0.14 + max(0.0, ovr - 82.0) * 0.42
    ...
    if age >= 35:
        ...
        # Young players are discounted for uncertainty, but the discount MUST scale
        # with talent and proven experience. A flat clamp branded 22-year-old stars
        # (OVR 90+) as ~$3.2M players — making their real deals look like gross
        # overpays and letting CPU/AI value them far below reality. Keep depth youth
        # cheap while letting elite, proven youngsters approach full market value.
...
def build_elc_contract(season_year: int, performance_bonus_m: float = 0.0) -> Dict[str, Any]:
...
def assign_elc_contract(
    ...
    if not slot_check.get("ok"):
        return {"ok": False, "reason": slot_check.get("reason")}

    contract = build_elc_contract(season_year)
    on_roster = player in _all_rostered(team)
    if on_roster:
        check = _validate_sign_cap(team, contract["aav_m"], league)
        if not check.get("ok"):
            return {"ok": False, "reason": check.get("reason"), "snapshot": check.get("snapshot")}
...
def qualifying_offer_aav(previous_aav_m: float) -> float:
    return round(max(LEAGUE_MINIMUM_AAV_M, float(previous_aav_m) * 1.05), 3)
...
def add_rfa_rights(team: Any, player: Any, season_year: int) -> Dict[str, Any]:
...
def evaluate_contract_offer(
    ...
    *,
    ...
    Interest is continuous (no reverse-engineerable cliffs, item 5) and depends
    only on the player's preferences — salary vs demand, term fit, age fit, and a
    small deterministic personality variance. Cap space is deliberately NOT part
    of interest (item 4): affordability is transaction eligibility (value 5),
    validated separately in sign_player_to_team.
    """
    aav_m = normalize_money_m(offer.get("aav_m") or offer.get("aav") or 0)
    years = max(1, int(offer.get("years") or offer.get("term") or 1))
    age = _player_age(player)

    demand = compute_player_demand(player, team, league, context=context)
    want_aav = demand["want_aav_m"]
    want_years = demand["want_years"]
    min_acceptable = demand["min_acceptable_aav_m"]
    prof = demand["profile"]
    security = prof["security_pref"]

    # --- Continuous interest (0..100) ---
    # Salary: smooth logistic around the player's want. r=0.90 -> ~0 delta,
    # rising to ~+40 as the offer meets/exceeds want, ~-40 well below. No cliffs.
    r = aav_m / max(0.25, want_aav)
    salary_component = 80.0 * (_logistic((r - 0.90) * 9.0) - 0.5)
    # Term: security-minded players reward hitting their preferred length more.
    term_gap = years - want_years
    term_component = 10.0 * math.tanh(term_gap * (0.5 + 0.5 * security))
    # Age/term fit: long deals sour older players; young studs like longer term.
    fit_component = 0.0
    if age >= 32:
        fit_component -= max(0, years - 3) * (2.5 + 3.0 * (1.0 - security))
    if age <= 24 and years >= 4:
        fit_component += 6.0
    variance_component = prof["variance"] * 100.0  # deterministic +/-5

    interest = 50.0 + salary_component + term_component + fit_component + variance_component
    interest = max(0.0, min(100.0, interest))

    accepted = interest >= 62.0 and aav_m >= min_acceptable

    if accepted:
        reason = "Accepted"
    elif aav_m < min_acceptable:
    ...
    # Trade-aware counter (item 6): the player will shave AAV toward his floor in
    # exchange for reaching his preferred term — a real give-and-take, not a flat
    # markup. Longer term buys more relief for security-minded players.
    counter_years = max(years, want_years)
    term_relief = 0.0
    if counter_years > years:
        term_relief = min(0.08, 0.02 * (counter_years - years) * (0.5 + security))
    counter_aav = round(max(min_acceptable, want_aav * (1.0 - term_relief)), 3)
    counter_aav = max(counter_aav, round(aav_m * 1.02, 3))

    # Projected cap is informational only (not an input to interest).
    ...
    return {
        ...
        "reason": reason,
        "counter_offer": {"aav_m": counter_aav, "years": counter_years},
        "risk_tags": risk_tags,
...
CPU_POSITION_OVERLOAD = {"C": 5, "LW": 5, "RW": 5, "LD": 5, "RD": 5, "G": 3}
...
def get_team_competitive_window(
    ...
    *,
    ...
    snap = get_team_cap_snapshot_full(team, league, sim, season_year=season_year)
    if float(snap.get("usable_cap_space_m", 0)) < 2.0:
        return "cap_strapped"
    slots_used = int(snap.get("contract_slots_used", 0))
    slots_limit = int(snap.get("contract_slots_limit", CONTRACT_SLOTS_LIMIT))
    if slots_limit - slots_used <= 2 and float(snap.get("usable_cap_space_m", 0)) < 5.0:
        return "cap_strapped"
    return window
...
def evaluate_team_position_needs(
    ...
    *,
    ...
    return {
        ...
        "cap_space_m": float(snap.get("usable_cap_space_m", 0)),
...
def cpu_signing_blocked(
    ...
    if ctx["overload"].get(pos) and ovr < best + 3.0:
    ...
    if window == "cap_strapped" and offer_aav > LEAGUE_MINIMUM_AAV_M + 0.15 and ovr < 76:
        return "cap_strapped_luxury"
    if ctx["slots_remaining"] <= 3 and ovr < 72 and float(ctx["need_score"].get(pos, 0)) < 0.35:
...
def sign_player_to_team(
    ...
    if bonus_m > 0:
        # Revenue floor: teams below $110M annual revenue cannot offer signing bonuses.
        revenue_m = None
        try:
        ...
        if revenue_m is None:
            try:
                ...
                # Session may be absent in unit paths — fall back to team attr only.
        ...
        if revenue_m < 110.0:
            return {
                "ok": False,
                "reason": f"Signing bonus not allowed — team revenue ${revenue_m:.1f}M is below $110M floor",
                "revenue_m": revenue_m,
            }
        # Bonus share of total contract value is capped by revenue tier.
        total_value = aav_m * years
        max_pct = 0.20 if revenue_m >= 180 else (0.14 if revenue_m >= 140 else 0.08)
    ...
    if not eval_result.get("accepted") and not offer.get("force"):
        return {"ok": False, "reason": eval_result.get("reason"), "evaluation": eval_result}
    ...
    sync_team_cap_fields(team, league)
    return {"ok": True, "player_id": _player_id(player), "contract": contract, "evaluation": eval_result}
...
def release_rfa_rights(team: Any, player_id: str, league: Any) -> Dict[str, Any]:
    entry = find_rfa_rights(team, player_id)
    if not entry:
        return {"ok": False, "reason": "RFA rights not found"}
    player = entry.get("player_ref")
...
def execute_offer_sheet(
    ...
    tier = "1st+3rd" if aav_m >= 6.5 else ("2nd" if aav_m >= 4.0 else "none")
    pending = {
        "player_id": _player_id(player),
...
def handle_player_contract_expiry(
    ...
    if "RFA" in rights or ctype == "RFA_BRIDGE":
        add_rfa_rights(team, player, season_year)
        _clear_expired_contract(player)
        return "rfa_rights"
...
def rebalance_team_cap_at_bootstrap(
    ...
    if payroll() > cap_m:
        non_core = [p for p in roster if id(p) not in protected and not has_true_elc_contract(p)]
...
def _cpu_negotiate_offer(
    ...
    *,
    ...
    if ceiling is None:
        ceiling = min(float(ctx.get("cap_space_m", 0)) * 0.35, compute_market_value(player, league) * 1.25)
    for _ in range(max(1, max_rounds)):
        ev = evaluate_contract_offer(
            player, team, {"aav_m": aav, "years": years, "context": context}, league, context=context
        )
        if ev.get("accepted"):
        ...
        if nxt_aav > ceiling:
            # Can't afford the money ask — try meeting the term ask instead, which
            # lowers the player's required AAV, before walking away.
            if nxt_years > years:
        ...
        for p in list(getattr(team, "prospect_pool", None) or []):
            ...
            if should_sign:
                ...
                if res.get("ok") and not res.get("already_signed"):
                    ...
                    actions.append({"team_id": tid, "player_id": _player_id(p), "action": "sign_elc"})
        prof = dict(prof)
...
def resolve_offer_sheets(session: Any) -> Dict[str, Any]:
    ...
    for sheet in sheets:
        ...
        comp = _record_offer_sheet_compensation(
            league, offering_team, rights_team, sheet.get("compensation_tier", "none"), season_year,
        )
...
def _record_offer_sheet_compensation(
    ...
    rounds = {"1st+3rd": [1, 3], "2nd": [2], "none": []}.get(str(tier or "none"), [])
...
def execute_cap_casualty_trade(
    ...
    *,
    ...
    audit = audit_pick_registry_integrity(league, start_year=season_year, years_ahead=4)
    critical = [
...
def build_contract_row(player: Any, team: Any, season_year: int, league: Any = None) -> Dict[str, Any]:
    ...
    return {
        ...
        "years_remaining": yrs,
        "yearsRemaining": yrs,
        "expiry_year": int(c.get("expiry_year") or season_year + yrs),
        "expiryYear": int(c.get("expiry_year") or season_year + yrs),
...
def build_rfa_rights_row(entry: Dict[str, Any], season_year: int) -> Dict[str, Any]:
    player = entry.get("player_ref")
    row = {
        "player_id": entry.get("player_id"),
        "name": entry.get("name"),
        ...
        "qualifying_offer_aav_m": entry.get("qualifying_offer_aav_m"),
    ...
    if player is not None:
        row.update(build_contract_row(player, None, season_year))
...
def build_contract_office(session: Any) -> Dict[str, Any]:
    from services.franchise_sim import _display_team, _ensure_team_roster_contracts_cap_safe, _build_free_agent_row
    ...
    if user_team is not None:
        heal_seed = abs(hash(f"contract_heal|{session.user_team_id}|{season_year}")) & 0xFFFFFFFF
        _ensure_team_roster_contracts_cap_safe(user_team, league, season_year, __import__("random").Random(heal_seed))
    ...
    rfa_rows = [build_rfa_rights_row(r, season_year) for r in _ensure_rfa_rights_list(user_team)] if user_team else []
    ...
    if user_team is not None and league is not None:
        ...
        # Cap goalie share so an OVR-sorted list is not flooded by goalies (whose pool
        # OVR bands sit above skaters). Keep the best goalies, fill the rest with skaters.
        LIST_LIMIT = 320
        goalie_cap = int(LIST_LIMIT * 0.16)
        goalies = [r for r in free_agents if str(r.get("position")) == "G"][:goalie_cap]
        skaters = [r for r in free_agents if str(r.get("position")) != "G"]
        free_agents = sorted(goalies + skaters[: LIST_LIMIT - len(goalies)], key=lambda r: -float(r.get("ovr") or 0))
...
def handle_contract_action(session: Any, action: str, body: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if action == "qualify-rfa":
        ...
        result = release_rfa_rights(user_team, player_id, league)
    ...
    if action == "buyout":
        ...
        promote = bool(body.get("promote_to_nhl") or body.get("promote"))
        try:
            ...
            result = attempt_sign_elc_with_decision(
                ...
                user_team,
                season_year=season_year,
                promote_to_nhl=promote,
                development_promise=body.get("development_promise"),
            )
        except Exception:
            result = sign_elc(p, user_team, league, season_year, promote_to_nhl=promote)
        ...
        return {
            "ok": True,
            "evaluation": evaluate_elc_signing_decision(
                p,
                user_team,
                season_year=season_year,
                promote_to_nhl=bool(body.get("promote_to_nhl") or body.get("promote")),
                development_promise=body.get("development_promise"),
            ),
        }
    elif action == "offer-sheet":
        rights_team = session.team_by_id.get(str(body.get("rights_team_id", "")))
        if player is None or rights_team is None:
...
def run_cap_compliance_before_season(session: Any, *, allow_rebalance: bool = False) -> Dict[str, Any]:
    ...
    return {"actions": actions, "pipeline": pipeline}
