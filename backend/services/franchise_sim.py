...
from app.sim_engine.league.schedule_generator import GameSlot, _safe_team_id
...
def _team_plays_on_day(by_day: Dict[int, List[Any]], day_idx: int, team_id: str) -> bool:
    ...
    for sl in by_day.get(int(day_idx), []) or []:
        if str(getattr(sl, "home_id", "") or "") == tid or str(getattr(sl, "away_id", "") or "") == tid:
            return True
    return False
...
def _can_place_user_game(
    ...
    probe = GameSlot(
        day=idx,
        home_id=str(user_id or ""),
        away_id=str(opp_id or ""),
        is_playoff=False,
    )
...
def _slot_key(slot: Any) -> Tuple[str, str, bool]:
    return (
        str(getattr(slot, "home_id", "") or ""),
        str(getattr(slot, "away_id", "") or ""),
        bool(getattr(slot, "is_playoff", False)),
    )


def _team_ids_for_slot(slot: Any) -> Tuple[str, str]:
    return (str(getattr(slot, "home_id", "") or ""), str(getattr(slot, "away_id", "") or ""))
...
def _validate_schedule_hard(
    ...
    *,
    ...
    for d in days:
        ...
        for sl in slots:
            hid = str(getattr(sl, "home_id", "") or "")
            aid = str(getattr(sl, "away_id", "") or "")
...
def _repair_regular_day_conflicts(
    ...
    for d in sorted(int(x) for x in list(by_day.keys())):
        ...
        for sl in slots:
            h = str(getattr(sl, "home_id", "") or "")
            a = str(getattr(sl, "away_id", "") or "")
...
def _franchise_team_abbrev(tm: Any) -> str:
    ...
    tid = str(getattr(tm, "team_id", None) or getattr(tm, "id", None) or rs._team_id(tm) or "")
...
def _scoring_chunk(
    ...
    for scorer, ng in Counter(scorers).items():
        ...
        nm = _name_str(scorer)
        high.append(f"{nm} ×{ng}" if ng > 1 else nm)
    return high
...
def _normalize_player_stat_row(row: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if is_goalie:
        ...
        gaa = (goals_against * 60.0) / max(1, gp * 60.0)
...
def _is_canadian_franchise_team(team: Any) -> bool:
    ...
    hints = ("toronto", "montréal", "montreal", "ottawa", "winnipeg", "edmonton", "calgary", "vancouver")
...
def _build_roster_browser(
    ...
    for t in getattr(league, "teams", None) or []:
        raw_tid = getattr(t, "team_id", None) or getattr(t, "id", None)
...
def _franchise_fanout_player_storylines(session: FranchiseSession, calendar_idx: int, day_meta: Dict[str, Any]) -> None:
    """Run one league-wide player storyline tick (catalog + psych/systemic effects) and surface to UI."""
    ...
    notes = [
        ...
        and str(n.get("type") or "") == "player_story"
        and int(n.get("date", -1)) == cur_date
    ]
    for row in rows:
        ...
        _record_storyline(
            session,
            {
                ...
                "player_name": pname,
                "players": [pname],
                "priority": "HIGH" if tier == "major" else "MEDIUM",
                "date": cur_date,
                "calendar_iso": iso,
            },
        )
        ...
        session.timeline.append(f"Storyline: {pname} ({abbrev}) — {short}")
        ...
        session.pending_decisions.append(
            {
                ...
                "description": (
                    ...
                    "Loan him to the national junior tournament roster (WJC recap only — no NHL club in the IIHF bracket), "
...
def _run_franchise_season_end_progression(session: FranchiseSession) -> Dict[str, Any]:
    ...
    session.season_calendar_year = sy + 1
    return out
...
def _enter_postseason(session: FranchiseSession) -> Dict[str, Any]:
    """Run year-end progression + playoff bracket once the NHL regular calendar is finished."""
    ...
    if int(prog.get("retired_removed", 0) or 0) > 0:
        session.notifications.append(f"{prog['retired_removed']} player(s) left active NHL rosters (retirement).")
    ...
    try:
        tbl = session.standings.league_table()
        utid = str(session.user_team_id)
        for rec in tbl[:32]:
            tid = str(rec.team_id)
            gp = max(1, int(getattr(rec, "gp", 0) or 0))
            try:
                pct = float(rec.point_pct())
            except Exception:
                pct = float(getattr(rec, "points", 0) or 0) / float(2 * gp)
            wn = int(getattr(rec, "wins", 0) or 0)
            tag = "neutral"
            if pct >= 0.62 and wn >= 48:
                tag = "dynasty_pace"
            elif pct <= 0.38 or wn <= 22:
                tag = "collapse_watch"
            tm = session.team_by_id.get(tid)
            if tm is not None:
                setattr(tm, "_season_dynamics_tag", tag)
            if tid == utid:
                _record_storyline(
                    session,
                    {
                        "type": "dynasty_tag",
                        "tone": "normal",
                        "date": str(session.season_calendar_year),
                        "headline": f"Season outlook tag: {tag.replace('_', ' ')}",
                        "team": utid,
                        "players": [],
                        "priority": "MEDIUM",
                    },
...
def advance_franchise_day(session: FranchiseSession) -> Dict[str, Any]:
    ...
    if world_injuries is not None and not _team_plays_on_day(
        session.by_day,
        idx,
    ...
    user_slots = [
        sl
        for sl in slots
        if str(getattr(sl, "home_id", "") or "") == utid
        or str(getattr(sl, "away_id", "") or "") == utid
    ]
