...
def _bump_stats_revision(session: FranchiseSession) -> None:
    ...
    session._cached_injuries_payload = None
...
def _narrative_cache_revision(session: FranchiseSession) -> int:
    return int(getattr(session, "_narrative_revision", 0) or 0)


def _bump_narrative_revision(session: FranchiseSession) -> None:
    session._narrative_revision = _narrative_cache_revision(session) + 1
    session._cached_narrative_universe_payload = None


def get_cached_narrative_universe_payload(session: FranchiseSession, *, force: bool = False) -> Dict[str, Any]:
    """Build narrative_universe once per revision — dominant cost in GET /state."""
    rev = _narrative_cache_revision(session)
    cached = getattr(session, "_cached_narrative_universe_payload", None)
    if (
        not force
        and isinstance(cached, dict)
        and int(cached.get("revision", -1)) == rev
        and int(cached.get("version", 0) or 0) == _NARRATIVE_UNIVERSE_CACHE_VERSION
    ):
    ...
    session._cached_narrative_universe_payload = {
        "revision": rev,
        "version": _NARRATIVE_UNIVERSE_CACHE_VERSION,
        "payload": payload,
    }
    return dict(payload) if isinstance(payload, dict) else {}


def _peek_cached_narrative_universe_payload(session: FranchiseSession) -> Dict[str, Any]:
    """Return warm narrative cache without rebuilding (used for crisis timer polls)."""
    rev = _narrative_cache_revision(session)
    cached = getattr(session, "_cached_narrative_universe_payload", None)
    if (
...
def _interaction_cache_revision(session: FranchiseSession) -> int:
    return int(getattr(session, "_interaction_revision", 0) or 0)


def _bump_interaction_revision(session: FranchiseSession) -> None:
    session._interaction_revision = _interaction_cache_revision(session) + 1
    session._cached_lean_state_payload = None
    session._cached_lean_state_key = None


def _build_narrative_summary(session: FranchiseSession) -> Dict[str, Any]:
    """Slim narrative slice for lean /state — no full universe rebuild."""
    pending = len(
        [
            row
...
def _get_cached_state_roster_rows(session: FranchiseSession, user_team: Any) -> List[Dict[str, Any]]:
    stats_rev = int(getattr(session, "_stats_revision", 0) or 0)
    cached = getattr(session, "_cached_state_roster_rows", None)
    if (
        isinstance(cached, dict)
        and int(cached.get("revision", -1)) == stats_rev
    ...
    if user_team is not None:
        for p in getattr(user_team, "roster", None) or []:
            if getattr(p, "retired", False):
                continue
            roster_rows.append(
                _serialize_player_row(p, include_ratings=True, session=session, _team=user_team)
            )
        roster_rows.sort(key=lambda x: -float(x.get("ovr") or 0))
    session._cached_state_roster_rows = {"revision": stats_rev, "payload": roster_rows}
    return roster_rows


def _get_cached_injuries_payload(session: FranchiseSession, *, limit: int = 200) -> List[Dict[str, Any]]:
    stats_rev = int(getattr(session, "_stats_revision", 0) or 0)
    injury_len = len(getattr(session, "injury_log_all", None) or [])
    cached = getattr(session, "_cached_injuries_payload", None)
    if (
        isinstance(cached, dict)
        and int(cached.get("stats_revision", -1)) == stats_rev
        and int(cached.get("injury_log_len", -1)) == injury_len
        and cached.get("payload") is not None
    ):
        return list(cached.get("payload") or [])
    payload = _build_injuries_payload(session, limit=limit)
    session._cached_injuries_payload = {
        "stats_revision": stats_rev,
        "injury_log_len": injury_len,
        "payload": payload,
    }
    return payload
...
def _lean_state_cache_key(session: FranchiseSession) -> str:
    ...
    return "|".join(
        [
            ...
            str(int(getattr(session, "_prospect_revision", 0) or 0)),
            str(_narrative_cache_revision(session)),
            str(_interaction_cache_revision(session)),
...
def _get_cached_nhl_calendar_full(
    ...
    *,
    ...
    stats_rev = int(getattr(session, "_stats_revision", 0) or 0)
    cur = int(getattr(session, "calendar_cursor", 0) or 0)
    if full or cursor_window is None:
        mode = "full"
    else:
        mode = f"win_{int(cursor_window[0])}_{int(cursor_window[1])}"
    cache = getattr(session, "_cached_nhl_calendar_slices", None) or {}
    entry = cache.get(mode) if isinstance(cache, dict) else None
    if isinstance(entry, dict) and int(entry.get("revision", -1)) == stats_rev and int(entry.get("cursor", -1)) == cur:
        payload = entry.get("payload")
        if isinstance(payload, list):
            return list(payload)
    payload = _nhl_calendar_full_with_slates(session, cursor_window=None if full else cursor_window)
    if not isinstance(getattr(session, "_cached_nhl_calendar_slices", None), dict):
        session._cached_nhl_calendar_slices = {}
    session._cached_nhl_calendar_slices[mode] = {"revision": stats_rev, "cursor": cur, "payload": payload}
    return payload


def _get_cached_user_cap_snapshot(
    session: FranchiseSession,
    user_team: Any,
    sim: Any,
    sy_cap: int,
) -> Tuple[Dict[str, Any], Dict[str, Any]]:
    default_info = {"salary_cap": 95.5, "cap_hit": 0.0, "cap_space": 95.5}
    ...
    return cap_snapshot_full, cap_info


def get_cached_trade_market_payload(session: FranchiseSession) -> Dict[str, Any]:
    rev = int(getattr(session, "_stats_revision", 0) or 0)
    cached = getattr(session, "_cached_trade_market_payload", None)
    if isinstance(cached, dict) and int(cached.get("revision", -1)) == rev:
        payload = cached.get("payload")
        if isinstance(payload, dict):
            return dict(payload)
    from services.trade_service import build_trade_market_payload  # noqa: WPS433

    payload = build_trade_market_payload(session)
    session._cached_trade_market_payload = {"revision": rev, "payload": payload}
    return dict(payload) if isinstance(payload, dict) else {}
        ...
        for src in list((getattr(session, "player_season_stats", None) or {}).values()):
            ...
            if player is not None:
                ...
                try:
                    ...
                    chapter_payload = serialize_chapter_profile_for_api(player)
                    if chapter_payload:
    ...
    if session is not None:
        try:
            ...
            if is_ahl:
                ...
                echl_compact = _ahl_light_season_stats(
                    p, session=session, is_goalie=(pos_str == "G"), team=_team, league_code="ECHL"
                )
                if nhl_compact and int(nhl_compact.get("gp", 0) or 0) > 0:
                    nhl_line = dict(nhl_compact)
                    nhl_line["league"] = "NHL"
                ...
                if echl_compact:
                    compact = echl_compact
                    affiliate = str(echl_compact.get("team_name") or "")
                    if affiliate:
                        row["team_name"] = affiliate
                        row["teamName"] = affiliate
                        row["affiliate_team_name"] = affiliate
            ...
            # Earlier stints (other leagues / earlier seasons) — synthetic minor/junior lines.
            _hist = getattr(p, "prospect_stat_history", None)
...
def _minor_league_team_display_name(team: Any, league_code: str) -> str:
    if str(league_code).upper() == "ECHL":
        for attr in ("echl_team_name", "echl_affiliate_name"):
            raw = getattr(team, attr, None) if team is not None else None
            if raw and str(raw).strip():
                return str(raw).strip()
        city = str(getattr(team, "city", "") or "").strip() if team is not None else ""
        name = str(getattr(team, "name", "") or "").strip() if team is not None else ""
        if name:
            return f"{city} {name} (ECHL)".strip() if city else f"{name} (ECHL)"
        return "ECHL Affiliate"
    return _ahl_affiliate_display_name(team)
...
def _ahl_light_season_stats(
    ...
    *,
    ...
    """Light season line for AHL / ECHL affiliate players.

    Minor-league games are not simulated player-by-player like the NHL schedule, so
    ...
    lines (deterministic per player, calendar-advanced) under an "AHL" / "ECHL"
    scoring profile so affiliate players show real basic totals instead of
    a blank stat line. The line belongs to the player's *current stint* only.
    """
    league_code = str(league_code or "AHL").upper()
    try:
    ...
    affiliate = _minor_league_team_display_name(team, league_code)
    out: Dict[str, Any] = {
        "gp": gp,
        ...
        "is_synthetic": True,
        "gp_missed": int(stats.get("gp_missed") or 0),
    }
    if is_goalie:
        out.update(
            {
...
def _merge_current_season_into_career_seasons(
    ...
    *,
    ...
    entry: Dict[str, Any] = {
        ...
        # Minor-league lines are model output, not simulated games; keep them
        # distinguishable from real imported NHL career rows.
        "is_synthetic": bool(compact.get("is_synthetic") or compact.get("is_ahl_synthetic")),
    }
    if is_goalie_row:
        entry.update(
            {
...
def build_draft_class_rankings(session: FranchiseSession, sim: Any) -> Dict[str, Any]:
    """Assemble the live franchise draft board from dev-league players.

    CANONICAL PATH: league_hierarchy_bootstrap (spawn) -> this function (board) ->
    draft_ranking_logic (scores/ETA) -> franchise_scouting (GM overlay). Do not use
    SimEngine prospect.py or draft_class_generator.py for live franchise rankings.
    ...
    for block in getattr(league, "development_leagues", None) or []:
        ...
        for tm in block.get("teams") or []:
            ...
            for p in tm.get("players") or []:
                ...
                # Draft class matches bootstrap shaping cohort (17–20; NCAA up to 24).
                # Age 16 kids are next-year depth, not this year's board.
                max_draft_age = 24 if str(code).upper() == "NCAA" else 20
                if age < 17 or age > max_draft_age:
                    continue
    ...
    from services.franchise_scouting import ensure_scouting_markers, get_hidden_ceiling_pool

    ensure_scouting_markers(session, board_prospects)
    hidden_ceiling_pool = get_hidden_ceiling_pool(session)
    _scout_month = None
    try:
        _cal = getattr(session, "nhl_calendar", None) or []
        _cur = int(getattr(session, "calendar_cursor", 0) or 0)
        if _cal and 0 <= _cur < len(_cal) and isinstance(_cal[_cur], dict):
            _scout_month = _cal[_cur].get("month")
    except Exception:
        _scout_month = None
    ...
    for i, row in enumerate(board_prospects):
        ...
        elif stock_mode == "weekly_heat":
            if signed_delta >= 4:
                trend, stock_label = "UP", "Hot Week"
            elif signed_delta >= 2:
                trend, stock_label = "UP", "Heating Up"
            elif signed_delta >= 1:
                trend, stock_label = "UP", "Warming"
            elif signed_delta <= -4:
                trend, stock_label = "DOWN", "Cold Week"
            elif signed_delta <= -2:
                trend, stock_label = "DOWN", "Cooling Off"
            elif signed_delta <= -1:
                trend, stock_label = "DOWN", "Fading"
            else:
        ...
        if isinstance(_scout_prospects, dict):
            ...
            if isinstance(_ov, dict) and _ov.get("scouted_percentage") is not None:
                try:
                    ...
                    scout_overlay_pct = 0.0
        _cv = compute_ceiling_visibility(
            rank,
            scout_overlay_pct,
            prospect_key=key,
            hidden_ceiling_pool=hidden_ceiling_pool,
            month=_scout_month,
        )
        ...
        # Ceiling movement is public only as a fogged hint: hidden ceilings show nothing, thin
        # files show direction only, deep files (your own scouting) also show the size.
        _pchg = row.get("_potential_change_true")
        if _pchg is not None and not ceiling_hidden and float(conf) >= 40.0 and abs(int(_pchg)) >= 1:
            _up = int(_pchg) > 0
            entry["potential_trend"] = "rising" if _up else "falling"
            entry["potential_trend_label"] = (
                "Scouts report the ceiling is rising" if _up else "Scouts report the ceiling is slipping"
            )
            if gm_scout_pct >= 60.0:
                entry["potential_change_est"] = int(_pchg)
        ...
        entry.update(
            {
                ...
                "public_ovr_low": pub_ovr["low"],
                "public_ovr_high": pub_ovr["high"],
                "public_ovr_spread": pub_ovr["spread"],
                "scout_disagreement_narrowing": pub_ovr["spread"],
        ...
        if isinstance(_ov, dict):
            if _ov.get("public_miss_type"):
                entry["public_miss_type"] = str(_ov.get("public_miss_type"))
            if _ov.get("spotlight"):
                entry["spotlight"] = True
            if _ov.get("film_study_complete"):
                entry["film_study_complete"] = True
            if _ov.get("miss_discovered"):
                entry["miss_discovered"] = True
        if ovr_revealed:
    ...
    return {
        ...
        "subtitle": f"{class_strength} ┬╖ draft-age (≤20) in dev leagues ┬╖ showing {len(entries)}",
        "total": total,
...
def _refresh_cpu_franchise_profiles(session: FranchiseSession, *, calendar_idx: int, force: bool = False) -> None:
    ...
    for tid, tm in (getattr(session, "team_by_id", None) or {}).items():
        ...
        ideology = evolved
        direction_upper = str(direction or "HOLDING").upper()
        if direction_upper in ("REBUILDING", "DEEP_REBUILD", "SELLER", "CAP_CORRECTION"):
            comp_window = "rebuild"
        ...
        profile = {
            ...
            "competitive_window": comp_window,
        ...
        try:
            ...
            win = str(profile.get("competitive_window") or "emerging").lower()
            if win in ("rebuild", "tank", "declining"):
                setattr(tm, "gm_window", "rebuild")
            elif win in ("contender",):
                setattr(tm, "gm_window", "contender")
            else:
...
def _trade_popup_num(v: Any, default: Optional[float] = None) -> Optional[float]:
    try:
        if v is None or v == "":
            return default
        return float(v)
...
def _trade_popup_prior_nhl_line(player: Any) -> Dict[str, Any]:
    """Most recent completed season with NHL games from stored career history (no invention)."""
    ...
    for row in reversed(seasons):
        if not isinstance(row, dict):
            continue
        league = str(row.get("league") or row.get("league_code") or "NHL").upper()
        gp = int(_trade_popup_num(row.get("gp"), 0) or 0)
        if league != "NHL" or gp <= 0:
            continue
        return dict(row)
    return {}


def _trade_popup_season_stats(
    session: Optional[FranchiseSession],
    player_id: str,
    player: Any = None,
) -> Dict[str, Any]:
    """Season line for a traded player, self-describing so the UI never has to guess.

    Returns {"kind": "skater" | "goalie", "source": ..., "label": ..., stats...} where source is
    "nhl_current", "minors" (AHL/ECHL/junior model line), "nhl_prior" (last NHL season on record)
    or "none". A player with no games gets source "none" and a note — never a row of zeros or dashes.
    """
    pid = str(player_id or "")
    is_goalie = bool(player is not None and _pos_str(player) == "G")
    row = dict((getattr(session, "player_season_stats", None) or {}).get(pid) or {}) if session is not None and pid else {}
    if str(row.get("position") or row.get("pos") or "").upper() == "G":
        is_goalie = True
    kind = "goalie" if is_goalie else "skater"
    season_y = int(getattr(session, "season_calendar_year", 0) or 0) if session is not None else 0
    season_label = f"{season_y}-{(season_y + 1) % 100:02d}" if season_y else ""
    gp = int(_trade_popup_num(row.get("gp"), 0) or 0)

    # 1) Current-season NHL games (real simulated stats).
    if row and gp > 0:
        compact = _compact_season_stats_for_player(session, pid) if session is not None else {}
        out: Dict[str, Any] = {"kind": kind, "source": "nhl_current", "label": f"{season_label} NHL".strip(), "gp": gp}
        if is_goalie:
            out.update(
                {
                    "w": compact.get("w"),
                    "l": compact.get("l"),
                    "sv_pct": compact.get("sv_pct"),
                    "gaa": compact.get("gaa"),
                    "so": int(_trade_popup_num(row.get("so", row.get("shutouts")), 0) or 0),
                }
            )
            return out
        g = int(row.get("g") or 0)
        a = int(row.get("a") or 0)
        out.update({"g": g, "a": a, "pts": int(row.get("pts") or (g + a)), "xgf_pct": None, "war": None})
        ...
        try:
            ...
            if xgf_sample > 0 or xgf_gp > 0:
                raw_xgf = enriched.get("xgf_pct")
                if raw_xgf is None:
                    raw_xgf = season_xgf_pct_from_row(row)
                if raw_xgf is not None:
                    xf = float(raw_xgf)
                    out["xgf_pct"] = round(xf * 100.0, 1) if xf <= 1.5 else round(xf, 1)
            if enriched.get("war") is not None:
                out["war"] = round(float(enriched.get("war")), 1)
        except Exception:
            try:
                if xgf_sample > 0:
                    out["xgf_pct"] = round((float(row.get("xgf") or 0) / xgf_sample) * 100.0, 1)
    ...
    ps = getattr(player, "_prospect_season_stats", None) if player is not None else None
    if isinstance(ps, dict) and int(_trade_popup_num(ps.get("gp"), 0) or 0) > 0:
        stint = getattr(player, "_prospect_stint", None)
        league_key = str((stint or {}).get("league_key") or "").upper() if isinstance(stint, dict) else ""
        asg = getattr(player, "_franchise_assignment", None)
        if not league_key and isinstance(asg, dict):
            league_key = str(asg.get("league_code") or asg.get("level") or "").upper()
        league_txt = league_key.replace("CHL_", "").replace("EU_J_", "") or "Minors"
        mg = int(_trade_popup_num(ps.get("gp"), 0) or 0)
        out = {"kind": kind, "source": "minors", "label": f"{season_label} {league_txt}".strip(), "gp": mg}
        if is_goalie:
            out.update(
                {
                    "w": int(_trade_popup_num(ps.get("wins"), 0) or 0),
                    "l": int(_trade_popup_num(ps.get("losses"), 0) or 0),
                    "sv_pct": _trade_popup_num(ps.get("save_pct")),
                    "gaa": _trade_popup_num(ps.get("gaa")),
                    "so": int(_trade_popup_num(ps.get("shutouts"), 0) or 0),
                }
            )
        else:
            g = int(_trade_popup_num(ps.get("goals"), 0) or 0)
            a = int(_trade_popup_num(ps.get("assists"), 0) or 0)
            out.update({"g": g, "a": a, "pts": int(_trade_popup_num(ps.get("points"), g + a) or (g + a))})
        return out

    # 3) Last completed NHL season on record (traded before playing this season).
    prior = _trade_popup_prior_nhl_line(player) if player is not None else {}
    if prior:
        pg = int(_trade_popup_num(prior.get("gp"), 0) or 0)
        label = str(prior.get("season") or "Last season")
        out = {"kind": kind, "source": "nhl_prior", "label": f"{label} NHL", "gp": pg}
        if is_goalie:
            out.update(
                {
                    "w": int(_trade_popup_num(prior.get("w", prior.get("wins")), 0) or 0),
                    "l": int(_trade_popup_num(prior.get("l", prior.get("losses")), 0) or 0),
                    "sv_pct": _trade_popup_num(prior.get("sv_pct", prior.get("svPct", prior.get("save_pct")))),
                    "gaa": _trade_popup_num(prior.get("gaa")),
                    "so": int(_trade_popup_num(prior.get("so", prior.get("shutouts")), 0) or 0),
                }
            )
        else:
            g = int(_trade_popup_num(prior.get("g", prior.get("goals")), 0) or 0)
            a = int(_trade_popup_num(prior.get("a", prior.get("assists")), 0) or 0)
            out.update({"g": g, "a": a, "pts": int(_trade_popup_num(prior.get("pts", prior.get("points")), g + a) or (g + a))})
        return out
    ...
    return {
        ...
        "label": season_label,
        "gp": 0,
        "note": "No games played this season",
    }
...
def _structured_trade_assets_from_execution(
    ...
    def _push(bucket: List[Dict[str, Any]], asset: Dict[str, Any]) -> None:
        ...
        if at in ("player", "") and (asset.get("player_name") or asset.get("player_id") or at == "player"):
            ...
            stats = _trade_popup_season_stats(session, pid, player)
...
def _compose_cpu_trade_wire_payload(
    ...
    *,
    ...
    return {
        ...
        "theme": "info",
        "icon": "⇄",
        # False for bulk-sim light payloads (names only); popups rebuild with full player data.
        "assets_resolved": bool(resolve_players),
    }
                    ...
                    "acquired_assets": [
                        {
                            ...
                            "season_stats": _trade_popup_season_stats(session, pid, player),
                        }
                    ],
                },
                {
...
def _enqueue_cpu_trade_popup(session: FranchiseSession, ev: Dict[str, Any], *, calendar_idx: int, iso: str) -> None:
    ...
    if not popup.get("assets_resolved", True):
        ...
        # players now (position, OVR, contract, season stats) instead of showing an empty stat row.
        try:
            popup = _compose_cpu_trade_wire_payload(
                session,
                ev,
                calendar_idx=calendar_idx,
                iso=iso,
                trade_id=str(popup.get("trade_id") or _trade_event_id(ev, calendar_idx)),
                resolve_players=True,
            )
        except Exception:
            pass
...
def _simulate_franchise_slot(session: FranchiseSession, slot: Any) -> Tuple[Optional[str], Optional[str]]:
    ...
    manual_hybrid = bool(getattr(session, "_manual_hybrid_light_cpu", False))
    ...
    cpu_only_light = (bulk_light or manual_hybrid) and not is_user_game
    cpu_only_bulk = cpu_only_light
    ...
    if session.use_world and world_momentum is not None:
        if cpu_only_bulk:
            h_scale = max(0.93, min(1.07, float(sim._roster_injury_depth_penalty(home))))
        ...
        elif session.prev_calendar_day is not None and d > session.prev_calendar_day:
            span = float(d - session.prev_calendar_day)
            world_momentum.decay_all_teams(teams, span * 0.06)
            session.prev_calendar_day = d

        if not cpu_only_bulk:
            if session.prev_calendar_day is None or d > session.prev_calendar_day:
                session.prev_calendar_day = d
            ...
            for block in getattr(league, "development_leagues", None) or []:
                ...
                for tm in block.get("teams") or []:
                    for p in tm.get("players") or []:
                        ...
                        if age <= max_age:
                            rows.append((p, code))
            # AHL / ECHL are NOT cached here: advance_all_development_league_stats reads the live
            # roster lists every run, so call-ups, send-downs and signings are seen immediately.
...
def _depth_pool_progression_tick(session: FranchiseSession) -> None:
    ...
    # Growing prospects are handled by the gradual prospect tick, not this one-shot lump.
    try:
        from services.prospect_in_season_growth import iter_growth_prospects
...
def _run_franchise_narrative_passes(
    ...
    *,
    ...
    """Daily narrative/storyline generation (day advance, bulk throttle, bulk catch-up)."""
    try:
...
def _franchise_narrative_should_run(
    ...
    *,
    ...
    if light_bulk and bulk:
        return False
    if not light_bulk and not bulk:
        return True
    if not light_bulk and bulk:
        return int(session.calendar_days_finished) % 3 == 0
    return False
...
def _bridge_universe_events_to_story_wire(session: FranchiseSession, *, limit: int = 32) -> int:
    """Promote recent universe interaction/life events onto the public storyline wire."""
    utid = str(getattr(session, "user_team_id", "") or "")
    emitted = 0
    existing = {
        str(ev.get("stable_key") or ev.get("id") or "")
        for ev in (getattr(session, "storyline_events", None) or [])
        if isinstance(ev, dict)
    }
    rows = list(getattr(session, "universe_interaction_queue", None) or [])
    rows += list(getattr(session, "universe_interactions", None) or [])[-80:]
    rows += list(getattr(session, "universe_event_log", None) or [])[-80:]
    for raw in reversed(rows):
        if emitted >= limit:
            break
        if not isinstance(raw, dict):
            continue
        tid = str(raw.get("team_id") or "")
        if utid and tid and tid != utid:
            continue
        headline = str(raw.get("headline") or raw.get("title") or "").strip()
        kind = str(raw.get("kind") or raw.get("type") or raw.get("event_type") or "")
        if not headline:
            pname = str(raw.get("player_name") or "Player")
            headline = f"{pname}: {kind.replace('_', ' ') if kind else 'universe event'}"
        stable = str(raw.get("stable_key") or raw.get("id") or f"univ|{headline[:120]}")
        if not stable or stable in existing:
            continue
        existing.add(stable)
        cause_type = str(raw.get("cause_type") or "").upper()
        category = str(raw.get("category") or "")
        if not category:
            if (
                kind.startswith("life_")
                or cause_type in ("MINOR_LIFE_EVENT", "POSITIVE_LIFE_EVENT")
                or "life" in kind.lower()
            ):
                category = "personal_life"
            else:
                category = kind or "locker_room"
        _record_storyline(
            session,
            {
                "type": category,
                "category": category,
                "cause_type": cause_type or kind.upper(),
                "headline": headline,
                "short_summary": str(raw.get("summary") or raw.get("description") or ""),
                "team": tid or utid,
                "team_id": tid or utid,
                "player_id": str(raw.get("player_id") or raw.get("actor_id") or ""),
                "player_name": str(raw.get("player_name") or ""),
                "priority": "MEDIUM",
                "heat": int(raw.get("heat") or 34),
                "stable_key": stable,
                "source": "universe_backfill",
            },
        )
        emitted += 1
    return emitted
...
def _maybe_backfill_sparse_storylines(session: FranchiseSession) -> None:
    ...
    days_finished = int(getattr(session, "calendar_days_finished", 0) or 0)
    if games < 10 and days_finished < 10:
        return
    raw = list(getattr(session, "storyline_events", None) or [])
    story_count = sum(1 for ev in raw if isinstance(ev, dict) and str(ev.get("headline") or "").strip())
    min_expected = max(6, min(48, max(games // 6, days_finished // 5)))
    if story_count >= min_expected:
        return
    cur_idx = int(getattr(session, "calendar_cursor", 0) or 0)
    sig = (cur_idx, story_count, games, days_finished)
    if getattr(session, "_narrative_sparse_backfill_sig", None) == sig:
        return
    setattr(session, "_narrative_sparse_backfill_sig", sig)
    days_advanced = max(days_finished, games // 2, 12)
    days_advanced = min(days_advanced, 30)
    ...
    *,
    ...
    if _franchise_narrative_should_run(session, just_idx=just_idx, light_bulk=light_bulk, bulk=bulk):
        _run_franchise_narrative_passes(
            ...
            just_idx,
            day_meta,
            full=not (light_bulk and bulk),
        )
        if not (light_bulk and bulk):
            _maybe_enqueue_post_day_decisions(session, user_lines)
        _bump_narrative_revision(session)
    # Light bulk defers game-box beats to advance_franchise_bulk catch-up.
    try:
        from app.sim_engine.league_hierarchy_bootstrap import tick_extra_league_development
...
def _snapshot_season_start_ovrs(session: FranchiseSession) -> None:
    ...
    try:
        # Dev-league + prospect-pool kids first; the AHL/ECHL pass below then wins for its own players.
        from services.prospect_in_season_growth import snapshot_prospect_season_start
...
def _prospect_in_season_development_tick(session: FranchiseSession) -> int:
    """Gradual OVR pulses + monthly potential review for prospects (dev leagues, pools, kids in the minors)."""
    from services.prospect_in_season_growth import prospect_in_season_tick

    res = prospect_in_season_tick(session)
    return int(res.get("moved", 0)) + int(res.get("potential", 0))
    ...
    *,
    ...
    for key, info in rank_changes.items():
        ...
        if entry_row:
            ...
            hist_row["rank"] = after
            hist_row["stock_mode"] = "rank_change"
            hist_row["stock_unit"] = "rank"
            hist_row["stock_delta"] = delta
            if delta > 0:
    ...
    *,
    ...
    for p in prospects:
        ...
        if str(row.get("prospect_classification") or "") == "drafted_user":
            ...
            # line so WJC development (skills + potential) applies to them too.
            dst = stats_by_id.get(str(row.get("player_id") or "")) or {}
            dgp = int(dst.get("gp") or 0)
            if dgp > 0:
                dpts = int(dst.get("pts") or 0)
                dgoals = int(dst.get("g") or 0)
                row["tournament_gp"] = dgp
                row["tournament_pts"] = dpts
                row["tournament_g"] = dgoals
                row["tournament_a"] = max(0, dpts - dgoals)
                row["tournament_plus_minus"] = int(dst.get("plus_minus") or 0)
                if str(dst.get("position") or row.get("position") or "").upper() == "G":
                    row["tournament_w"] = int(dst.get("w") or 0)
                    row["tournament_l"] = int(dst.get("l") or 0)
                    row["tournament_shutouts"] = int(dst.get("shutouts") or 0)
                    row["tournament_sv_pct"] = dst.get("sv_pct")
            out.append(row)
            continue
        pid = str(row.get("player_id") or "")
...
def advance_franchise_day(session: FranchiseSession) -> Dict[str, Any]:
    ...
    cap_sy = int(getattr(session, "season_calendar_year", 2025) or 2025)
    if not bulk_light or int(getattr(session, "_bulk_cap_season_year", -1) or -1) != cap_sy:
        ensure_session_nhl_salary_cap(session)
        if bulk_light:
            session._bulk_cap_season_year = cap_sy
    ...
    if (
        not bulk_light
        and world_injuries is not None
    ...
    session._manual_hybrid_light_cpu = True
    try:
        user_lines, league_lines = _simulate_slots_for_day(session, idx, slots)
    finally:
        session._manual_hybrid_light_cpu = False
...
def _build_state_payload_impl(session: FranchiseSession, *, include_heavy: bool = False, crisis_tick: bool = False) -> Dict[str, Any]:
    ...
    if include_heavy:
        try:
            narrative_universe = get_cached_narrative_universe_payload(session)
        except Exception:
            narrative_universe = {}
    try:
        from services.trade_demand_engine import (  # noqa: WPS433
    ...
    if postseason_lean:
        ...
        nhl_calendar_full = _get_cached_nhl_calendar_full(session, full=True)
    else:
        nhl_calendar_full = _get_cached_nhl_calendar_full(session, cursor_window=(21, 14))
    ...
    payload = {
        ...
        "narrative_universe": narrative_universe if include_heavy else {},
        "trade_demand_crisis": trade_demand_crisis,
        "trade_deadline": trade_deadline,
        "trade_stability_roster_flags": dict(getattr(session, "trade_stability_roster_flags", None) or {}),
        "conduct_org_pressure": dict(getattr(session, "_conduct_org_pressure", None) or {}),
    ...
    try:
        ...
        extras = build_offseason_state_extras(session, lean=not include_heavy, hydrate_stages=False)
        extra_flags = dict(extras.pop("flags", {}) or {})
        payload.update(extras)
...
def invalidate_session_payload_caches(session: FranchiseSession, reason: str = "") -> None:
    ...
    if reason == "draft_pick" or reason == "draft_pick_trade":
        # Selection / pick trade only mutates rights / drafted set. Clear roster/scouting views
    ...
    narrative_reasons = {
        ...
        "player_meeting_start",
        "player_meeting_advance",
        "storyline_choice",
        "trade_exec",
        "burner_post",
        "narrative_tick",
    }
    if reason in narrative_reasons:
        _bump_narrative_revision(session)
        session._cached_narrative_universe_payload = None
    elif reason not in ("advance_day", "game_stats", "player_stats", "season_reset"):
        session._cached_narrative_universe_payload = None
    session._cached_nhl_calendar_slices = None
    session._cached_user_cap_snapshot = None
    session._cached_trade_market_payload = None
    session._cached_contract_office_payload = None
    if reason in (
        "trade_exec",
        "game_stats",
        "season_reset",
...
def get_franchise_chemistry_report(session: FranchiseSession) -> Dict[str, Any]:
    ...
    if isinstance(cached, dict) and str(cached.get("key") or "") == cache_key:
        ...
        if isinstance(payload, dict):
            return dict(payload)
    payload = build_public_chemistry_report(session)
    session._cached_chemistry_report = {"key": cache_key, "payload": payload}
    return payload
